...
export class RandomGen3Teams extends RandomGen4Teams {
	...
	override getItem(
		...
		if (['healingwish', 'switcheroo', 'trick'].some(m => moves.has(m))) {
			if (
				species.baseStats.spe >= 60 && species.baseStats.spe <= 110 && role !== 'Wallbreaker' && !counter.get('priority')
			) {
				return 'Choice Scarf';
			} else {
	...
	override randomTeam() {
		...
		while (baseSpeciesPool.length && pokemon.length < this.maxTeamSize) {
			...
			if (set.moves.includes('spikes') || set.moves.includes('stealthrock')) teamDetails.spikes = 1;
			if (set.moves.includes('rapidspin') || set.moves.includes('defog') || set.moves.includes('tidyup')) teamDetails.rapidSpin = 1;
