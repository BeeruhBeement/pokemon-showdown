...
export class RandomGen3Teams extends RandomGen4Teams {
	...
	constructor(format: string | Format, prng: PRNG | PRNGSeed | null) {
		...
		this.moveEnforcementCheckers = {
			Bug: (movePool, moves, abilities, types, counter, species) => !counter.get('Bug'),
			...
			Steel: (movePool, moves, abilities, types, counter, species) => !counter.get('Steel'),
	...
	getItem(
		...
		if (species.id === 'pinsir' && ability === 'Chrysalis'){
			if(moves.has('curse')) return 'White Herb'
		...
		if (counter.get('Physical') >= 3 && (moves.has('batonpass') || moves.has('uturn') || moves.has('voltswitch') || moves.has('flipturn') || moves.has('teleport'))) {
			return 'Choice Band';
		}
		if (counter.get('Special') >= 3 && ((moves.has('batonpass') || moves.has('uturn') || moves.has('voltswitch') || moves.has('flipturn') || moves.has('teleport')))) {
			return 'Choice Specs';
		}
