...
export class RandomGen3Teams extends RandomGen4Teams {
	...
	cullMovePool(
		...
		const badWithSetup = ['knockoff', 'toxic'];
	...
	getItem(
		...
		if (species.id === 'linoone' && role === 'Setup Sweeper') return 'Aguav Berry';
		if (species.id === 'marowak') return 'Thick Club';
		if (species.id === 'pikachu' || species.id === 'raichu') return 'Light Ball';
		if (species.id === 'shedinja') return 'Focus Sash';
		if (species.id === 'shuckle') return 'Leftovers';
		if (species.id === 'unown') return counter.get('Physical') ? 'Choice Band' : 'Choice Specs';
		if (ability === 'Poison Heal' || ability === 'Toxic Boost' || (moves.has('facade') && ability !== "Guts")) return 'Toxic Orb';
		...
		if (counter.get('Special') >= 4) return 'Choice Specs';

		if (
			species.baseStats.spe >= 60 && species.baseStats.spe <= 130 && role !== 'Wallbreaker' && !counter.get('priority') && (counter.get('Physical') >= 3 || counter.get('Special') >= 3)
		) {
			return 'Choice Scarf';
		}
