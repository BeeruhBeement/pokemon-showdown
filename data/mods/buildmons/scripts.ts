...
export const Scripts: ModdedBattleScriptsData = {
	...
	pokemon: {
		...
		setStatus(
			...
			if (
				!ignoreImmunities && status.id && !((source?.hasAbility('corrosion') && ['tox', 'psn'].includes(status.id) || (sourceEffect && sourceEffect.id === 'pyromaniac')))
			) {
		...
		getMoves(lockedMove?: string | null, restrictData?: boolean): MoveRequestData[] {
			if (lockedMove) {
		...
		getCappedBoost(boosts: SparseBoostsTable) {
			...
			for (boostName in boosts) {
				...
				if (!boost) continue;
				cappedBoost[boostName] = this.battle.clampIntRange(this.boosts[boostName] + boost, 0, 500) - this.boosts[boostName];
			}
			return cappedBoost;
		},
	},
