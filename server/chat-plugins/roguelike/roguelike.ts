...
type ItemType = 'pokemonPack' | 'healHP' | 'healPP' | 'TM' | 'abilityCapsule' |
...
interface AbilityCapsuleItem extends RotationalItem {
    ability: string;
}
...
// generate a tm of every move
const TM_LIST: { [k: string]: TMItem } = {};
for (const move of Dex.moves.all()) {
    if (move.isNonstandard || move.placeholderFor || move.isZ || move.isMax || move.id === 'struggle') continue;
    const tmId = `tm_${move.id}`;
    TM_LIST[tmId] = {
        name: `TM - ${move.name}`,
        move: move.name,
        cost: 3,
        icon: 'item-tm',
        type: 'TM',
        desc: `Teaches the move ${move.name} to any Pokemon.`,
        minStreak: 0,
    };
}

const ABILITY_CAPSULE_LIST: { [k: string]: AbilityCapsuleItem } = {};
for (const ability of Dex.abilities.all()) {
    if (ability.isNonstandard) continue;
    const capId = `abilitycapsule_${ability.id}`;
    ABILITY_CAPSULE_LIST[capId] = {
        name: `Ability Capsule - ${ability.name}`,
        ability: ability.name,
        cost: 4,
        icon: 'item-ability-capsule',
        type: 'abilityCapsule',
        desc: `Changes a Pokemon's ability to ${ability.name}.`,
        minStreak: 0,
    };
}

let loadedItemDb: { [k: string]: RotationalItem | TMItem | AbilityCapsuleItem } = JSON.parse(FS(`${ROGUELIKE_DATA_PATH}/itemdb.json`).readSync());

const ROTATIONAL_ITEM_POOL: { [k: string]: RotationalItem | TMItem | AbilityCapsuleItem } = Object.assign(
    {},
    loadedItemDb,
    TM_LIST,
    ABILITY_CAPSULE_LIST
);
...
function refreshPage(userID: ID) {
    ...
    for (const conn of realUser.connections) {
        // Only refresh the view if the user already has the page open
        if (conn.openPages?.has('roguelike')) {
...
export class Roguelike {
	...
	rollShop() {
		...
		for (const index of Object.keys(ROTATIONAL_ITEM_POOL)) {
			const itemObj = ROTATIONAL_ITEM_POOL[index];
			if (itemObj.type === 'item') {
				const dexItem = Dex.items.get(itemObj.name);
				...
				if (dexItem.itemUser && !this.team.some(p => dexItem.itemUser?.includes(p.species))) continue;
				if (!isViable) continue;
			} else if (itemObj.type === 'evolveItem') {
				const dexItem = Dex.items.get(itemObj.name);
				if (!this.team.some(p => checkForEvolution(p, dexItem.name))) continue;
			}
		...
		while (this.rotationalShop.length < 5) {
			...
			if (randomNo > 45) {
				...
				potential = viableItems.filter(item => ROTATIONAL_ITEM_POOL[item].type === 'abilityCapsule' ||
	...
	healTeam(percentage: number) {
		for (const monData of this.teamData) {
			if (!monData.maxHP) continue;
			const healAmount = Math.max(1, Math.floor(monData.maxHP * percentage));
			if (monData.curHP <= 0) {
				monData.curHP = healAmount;
			} else {
				monData.curHP = Utils.clampIntRange(monData.curHP + healAmount, 1, monData.maxHP);
			}
			if (monData.status === 'fnt') {
				monData.status = false;
			}
		}
	}
	...
	genUserTeamHTML() {
		...
		buf += `<table style="width:100%; border-collapse: collapse;"border="1"><tr><th>Status</th><th>Info</th><th>Moves</th><th>Actions</th></tr>`;
		let linkedIndex = 0;
		for (const mon of this.team) {
			...
			buf += `<br /><br /><button class="button" name="send" value="/roguelike managemoves ${linkedIndex + 1}">Manage Moves</button>`;
			if (mon.item) {
				buf += `<br /><br />`;
	...
	genQuickSelectHTML(checkItem: ItemType | "switch" | "switchitem", targetIndex?: number) {
		...
		for (const mon of this.team) {
			...
			case 'TM':
				failureCondition = mon.moves.includes(this.flags.moveToLearn);
				cmd = `redeem tm, ${index}`;
				skipmsg = 'Undo';
				break;
			case 'abilityCapsule':
				failureCondition = (mon.ability === this.flags.abilityToLearn);
				cmd = `redeem abilitycapsule, ${index}`;
				skipmsg = 'Undo';
				break;
			case 'key':
			case 'debug':
	...
	genPurchaseHTML(failure?: boolean) {
		...
		case 'TM':
			...
			return buf;
		case 'abilityCapsule':
			buf = `<center>Change Ability to ${this.flags.abilityToLearn} for what Pokemon?</h3></center><br />`;
			buf += this.genQuickSelectHTML((this.flags.purchasedItem as ShopItem)?.type);
			return buf;
		case 'key':
			break;
...
function createSaveData(user: User) {
	...
	// Gen starters here
	rl.flags.pokemonOptions = genPokemon(4, 5, { midpoint: 315, range: 65, weightcap: 100 }, true);
...
export const commands: Chat.ChatCommands = {
	...
	roguelike: {
		...
		managemoves(target, room, user) {
			...
			if (Number.isNaN(index) || index < 1 || !userData.team[index - 1]) throw new Chat.ErrorMessage(`Invalid pokemon index.`);
			userData.goToPage(`shop-managemoves-${index}`);
		},
		initiatelearn(target, room, user) {
			const userData = roguelikeGames.get(user.id);
			if (!userData || userData.runEnded) throw new Chat.ErrorMessage(`You need to make a new run first.`);
			if (!checkSequence(userData.curRoom, 'shop')) throw new Chat.ErrorMessage(`Can't do that here!`);
			if (!target) throw new Chat.ErrorMessage(`You need to specify a pokemon and a move.`);
			const args = target.split(',');
			let arg = args.shift();
			...
			if (!move) throw new Chat.ErrorMessage(`You need to specify a move.`);
			// compute level-up learnable moves for this pokemon up to its current level
			const mon = userData.team[index];
			const learnable = getLevelUpMovesUpTo(mon.species, mon.level || 100);
			if (!learnable.includes(toID(move))) {
				// also allow passing move ids; normalize
				if (!learnable.includes(move)) throw new Chat.ErrorMessage(`That move is not available to learn right now.`);
			}
			const moveName = Dex.moves.get(move).name;
			// don't allow already-known moves
			if (userData.team[index].moves.some(m => toID(m) === toID(moveName))) throw new Chat.ErrorMessage(`This pokemon already knows ${moveName}.`);
			userData.flags.moveToLearn = moveName;
			userData.flags.pokemonForTM = index;
			// If space available, learn immediately
		...
		buy(target, room, user) {
			...
			case 'TM':
				...
				userData.flags.isRotationalItem = true;
				userData.flags.purchasedItem = item;
				userData.flags.purchasedItemKey = target;
				userData.battlePoints -= item.cost;
				break;
			case 'abilityCapsule':
				userData.flags.abilityToLearn = (item as AbilityCapsuleItem).ability;
				userData.flags.isRotationalItem = true;
				userData.flags.purchasedItem = item;
				userData.flags.purchasedItemKey = target;
				userData.battlePoints -= item.cost;
		...
		addstarter(target, room, user) {
			...
			userData.flags.pokemonOptions = pokes.filter(p => p !== poke);
			if (userData.team.length < 2) {
				userData.goToPage('intro');
				return;
			}
			userData.opponentTeam = userData.flags.pokemonOptions.slice();
		...
		redeem(target, room, user) {
			...
			case 'tm':
				...
				index--;
				if (!userData.team[index]) throw new Chat.ErrorMessage(`You need to specify a pokemon on your team.`);
				if (userData.team[index].moves.includes(userData.flags.moveToLearn)) {
					throw new Chat.ErrorMessage(`You can't use this on that pokemon.`);
				}
				userData.flags.pokemonForTM = index;
				if (userData.team[index].moves.length >= 4) {
			...
			case 'abilitycapsule':
				...
				index--;
				if (!userData.team[index]) throw new Chat.ErrorMessage(`You need to specify a pokemon on your team.`);
				if (userData.team[index].ability === userData.flags.abilityToLearn) {
					throw new Chat.ErrorMessage(`You can't use this on that pokemon.`);
				}
				userData.team[index].ability = userData.flags.abilityToLearn;
				if (userData.flags.isRotationalItem && userData.flags.purchasedItemKey) {
					const shopIndex = userData.rotationalShop.indexOf(userData.flags.purchasedItemKey);
					if (shopIndex >= 0) userData.rotationalShop.splice(shopIndex, 1);
					delete userData.flags.isRotationalItem;
				}
				delete userData.flags.abilityToLearn;
				if (userData.flags.purchasedItem) delete userData.flags.purchasedItem;
				delete userData.flags.purchasedItemKey;
				userData.goToPage('shop');
				return;
			default:
			...
			if (target === 'done') {
				...
				if (userData.curRoom.endsWith(`-done`)) {
					...
					if (userData.flags.purchasedItem) {
						const purchasedItemType = (userData.flags.purchasedItem as ShopItem).type;
						if (purchasedItemType !== 'TM' && purchasedItemType !== 'abilityCapsule') {
							userData.battlePoints -= (userData.flags.purchasedItem as ShopItem).cost;
						}
	...
	roguelike(args, user) {
		...
		case 'shop':
			...
			case 'team':
				...
				break;
			case 'managemoves': {
				subtitle = 'Manage Moves';
				buf += `<button class="button" name="send" value="/roguelike shop">Go back to shop</button>`;
				buf += `<br /><br /><button class="button" name="send" value="/roguelike checkteam">Check your team</button>`;
				const manageIndex = gameArgs.shift();
				if (!manageIndex) throw new Chat.ErrorMessage('If you reached this error, contact HiZo.');
				const manageNumber = parseInt(manageIndex);
				if (Number.isNaN(manageNumber) || manageNumber < 1 || !userGameData.team[manageNumber - 1]) throw new Chat.ErrorMessage('Invalid pokemon index.');
				const relevant = userGameData.team[manageNumber - 1];
				const relevantData = userGameData.teamData[manageNumber - 1];
				buf += `<center><h3>Manage moves for ${relevant.name}</h3><br />`;
				buf += `<psicon pokemon=${relevant.species}><br /><br />`;
				let mi = 0;
				for (const m of relevant.moves) {
					const dm = Dex.moves.get(m);
					buf += `${dm.name}: ${relevantData.ppLeft[mi]}/${dm.pp * (8 / 5)}<br />`;
					mi++;
				}
				const learnable = getLevelUpMovesUpTo(relevant.species, relevant.level || 100)
					.filter(m => {
						const moveName = Dex.moves.get(m).name;
						return !relevant.moves.some(rm => toID(rm) === toID(moveName));
					});
				if (learnable.length) {
					buf += `<br /><b>Learnable moves (level-up):</b><br />`;
					for (const pm of learnable) {
						const moveName = Dex.moves.get(pm).name;
						buf += `<button class="button" name="send" value="/roguelike initiatelearn ${manageNumber},${pm}">Learn ${moveName}</button>&nbsp;`;
					}
				}
				buf += `</center>`;
				break;
		...
		case 'intro':
			...
			if (userGameData.team.length === 0) {
				buf += `<center><h3>Choose your first starter!</h3><br />`;
			} else {
				buf += `<center><h3>Choose your second starter!</h3><br />`;
			}
...
export const handlers: Chat.Handlers = {
	...
	onBattleEnd(battle, winner, players) {
		...
		if (humanGameData.teamData.some(poke => poke.pendingLevelUpMoves?.length)) {
			humanGameData.goToPage('learnmoves');
		} else if (humanGameData.teamData.some(poke => !!poke.evoFlag)) {
